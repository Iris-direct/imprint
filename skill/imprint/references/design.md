...
- **Give the page a favicon.** A browser tab showing the default globe undoes a lot of the care in the rest of the page, and the tab is the first thing a person sees when they open their own card. Design a tiny mark in the card's own palette — a monogram of their initial, or a glyph from the visual world you chose — and inline it as a data-URI SVG so the card stays self-contained: `<link rel="icon" href="data:image/svg+xml,...">`. Check it at 16px: at that size only a bold shape and one letterform survive, so drop any detail that turns to mush.
